...
function In_SFnp(vn::Float64, vp::Float64, xi::Float64, n::Int64)
    ...
    yp=vp
    integ_arr = Array{Float64,1}[]
    for ctn=ctns
        integ_arr_tmp = Array{Float64,1}[]
        for phi1=phi1s
            ...
            for inu=1:n, i1=1:n, i2=1:n, i3=1:n, i4=1:n
                ...
                integ_tmp .+= (wnu*w1*w2*w3*w4 * exp(xnu+x1+x2+x3+x4)) .* integrand_In_SFnp(xnu, x1, x2, x3, x4, yn, yp, yn1, yn2,xi)./ (2*pi)
            end
