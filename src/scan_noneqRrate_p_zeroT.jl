...
for iy in 1:ysize
    for ix in 1:xsize
        Rs[iy,ix] = Ip_rate_SFnp_trp(dn[ix], dp[iy], xi, 10) / HM(xi)
    end
    println(iy, ", ")
end

open("Rrate_murca_p_xarray.dat", "w") do f
...
open("Rrate_murca_p_yarray.dat", "w") do f
